...
class Calculations:
    def __init__(self):
        ...
        self.lib.Mores_integral.argtypes = [
    ...
    def calc(self, model, sup1, sup2):
        ...
        for load in model.data.get("loads"):
            if isinstance(load, DistributedForce):
                distributed_loads.append([
                    load.node.direction.x - load.lenght / 2,
                    load.node.direction.x + load.lenght / 2,
                    load.direction.y,
                ])
        ...
        point_loads_m = self.create_matrix(point_loads)
        distributed_loads_m = self.create_matrix(distributed_loads)
        moments_m = self.create_matrix(moments)
        ...
        # Вызываем функцию
        self.lib.diagram_calc(
            x,
            sup1.node.direction.x,
            sup2.node.direction.x,
            x.size,
            v,
            m,
            point_loads_m,
        ...
        self.lib.Matrix_destroy(point_loads_m)
        self.lib.Matrix_destroy(distributed_loads_m)
        self.lib.Matrix_destroy(moments_m)

        return v, m

    @staticmethod
    def determinate_system():
        pass

    @staticmethod
    def undeterminate_system():
        pass

    def mores_integral(self, size, M1Mes, M1Mb):
        a = [
            [self.lib.Mores_integral(ep1, ep2, size, float(config("DX"))) for ep2 in M1Mes]
            for ep1 in M1Mes
        ]
    ...
    def solve(self, A, B):
        ...
        X = [] + [self.lib.Matrix_get(res, i, 0) for (i, _) in enumerate(B)]
